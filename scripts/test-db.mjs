...
for (const file of ['production_crm.test.sql', 'team_management.test.sql', 'qr_intake.test.sql']) {
...
const concurrency = spawnSync(process.execPath,['scripts/test-qr-concurrency.mjs'],{stdio:'inherit',env:process.env})
if (concurrency.status !== 0) process.exit(1)
