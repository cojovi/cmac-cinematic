...
function Logo({ small = false }: { small?: boolean }) {
  return (
    <a aria-label="CMAC Roofing home" className={small ? 'logo logo-small' : 'logo'} href="#">
...
function RedButton({ children, wide = false }: { children: React.ReactNode; wide?: boolean }) {
  return (
    <a className={wide ? 'btn btn-red btn-wide' : 'btn btn-red'} href="#inspection">
      {children}
...
function IconBox({ children, soft = false }: { children: React.ReactNode; soft?: boolean }) {
    ...
    <main className="site-shell">
      <header className="top-nav">
        ...
        <nav className={menuOpen ? 'nav-links nav-links-open' : 'nav-links'} aria-label="Primary">
          <a href="#services">Services <ChevronDown size={10} /></a>
          <a href="#process">Our Process</a>
          <a href="#footer">About Us</a>
          <a href="#states">Locations</a>
          <a href="#footer">Resources <ChevronDown size={10} /></a>
          <a href="#footer">Careers</a>
        </nav>
        <div className="nav-actions">
          <a className="phone-pill" href="tel:8332623222"><Phone size={12} /> (833) 262-3222</a>
          <a className="nav-request" href="#inspection">Request Inspection</a>
        </div>
      ...
      <section className="hero">
        ...
        <div className="hero-copy">
          <h1>CMAC<br />ROOFING</h1>
          <p className="hero-sub">Veteran-owned. Mission-focused.<br />Protecting what matters across 6 states.</p>
          <p className="hero-body">Roofing, gutters, doors, and restoration built to endure—backed by discipline, integrity, and unmatched quality.</p>
        ...
        <form id="inspection" className="inspection-card">
          ...
          <button type="button">Schedule Inspection <ArrowRight size={13} /></button>
        ...
        <div className="trust-row">
          {trustBadges.map(({ title, detail, Icon }) => (
            <div className="trust-item" key={title}>
              <IconBox soft><Icon size={15} /></IconBox>
      ...
      <section id="states" className="panel states-panel">
        ...
        <div className="states-copy">
          ...
          <h2>Proudly Serving<br />6 States</h2>
          <ul>
            <li>Texas</li>
            <li>Tennessee</li>
            <li>Oklahoma</li>
            <li>Georgia</li>
            <li>Arkansas</li>
            <li className="dim">Expanding to serve<br />more communities.</li>
          </ul>
        </div>
        <aside className="local-card">
          <IconBox soft><MapPinned size={20} /></IconBox>
          <p>Local teams.<br />Rapid response.<br />Built for your region.</p>
          <a href="#footer">View All Locations <ArrowRight size={11} /></a>
        </aside>
      </section>
        ...
        <div className="service-grid">
          {services.map(({ title, body, image, link, Icon }) => (
            <article className="service-card" key={title}>
              ...
              <IconBox soft><Icon size={17} /></IconBox>
              <div>
                <h3>{title}</h3>
                <p>{body}</p>
                <a href="#inspection">{link} <ArrowRight size={11} /></a>
              </div>
            </article>
          ))}
        </div>
      ...
      <section id="process" className="panel process-panel">
        <div className="process-title">
          ...
          <h2>Built on a<br />Disciplined Process</h2>
        </div>
      ...
      <section className="panel press-panel">
        <div>
          ...
          <h2>Recognized for<br />Excellence</h2>
        </div>
        <div className="press-logos">
          <span className="forbes">Forbes</span>
          <span className="entrepreneur">Entrepreneur</span>
          <span className="inc">Inc.</span>
          <span className="roofing-logo">ROOFING<br />CONTRACTOR</span>
          <span className="yahoo">yahoo!<small>finance</small></span>
        </div>
      </section>
      ...
      <section className="panel reviews-panel">
        <div className="review-title">
          ...
          <h2>Real Reviews.<br />Real Results.</h2>
        </div>
        <button className="arrow-control left" aria-label="Previous review"><ChevronLeft size={15} /></button>
          ...
          {reviews.map((review) => (
            <article className="review-card" key={review.name}>
              ...
              <div className="review-person">
                ...
                <span><b>{review.name}</b><em>{review.city}</em></span>
              </div>
            </article>
          ))}
        </div>
        <button className="arrow-control right" aria-label="Next review"><ChevronRight size={15} /></button>
      </section>
      ...
      <section className="cta-panel">
        ...
        <div className="cta-copy">
          ...
          <h2>Let’s Protect<br />What Matters.</h2>
        ...
        <div className="benefit-card">
          <article>
            ...
            <p>Book online in<br />60 seconds.</p>
          </article>
          <article>
            <Shield size={31} />
            <h3>No Obligation</h3>
            <p>Free inspection<br />& honest answers.</p>
          </article>
          <article>
            <Headphones size={31} />
            <h3>Quick Response</h3>
            <p>We respond within<br />15 minutes.</p>
          </article>
          <div className="benefit-actions">
            <RedButton wide>Request Inspection</RedButton>
            <a className="btn btn-outline btn-wide" href="tel:8332623222"><Phone size={12} /> (833) 262-3222</a>
          </div>
        </div>
      </section>
      ...
      <footer id="footer" className="footer-panel">
        ...
        <div className="footer-bottom">
          ...
          <span><a href="#footer">Privacy Policy</a><a href="#footer">Terms of Service</a></span>
        </div>
      </footer>
    </main>
  )
