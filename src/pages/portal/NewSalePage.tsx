...
export default function NewSalePage() {
  ...
  const selectedProjectAddress = typeof selectedContact?.project_address === 'string' ? selectedContact.project_address : null
  const selectedDisplayName = typeof selectedContact?.display_name === 'string' ? selectedContact.display_name : null
  ...
  const completion = useMemo(() => Math.round(([Boolean(unit), contactReady, Boolean(dealId)].filter(Boolean).length / 3) * 100), [contactReady, dealId, unit])
  ...
  async function saveDraft() {
    ...
    try {
      ...
      if (dealId) {
        const { error: updateError } = await supabase.from('deals').update({ contact_id: savedContactId, base_amount: unit.price, delivery_amount: deliveryAmount, deposit_percent: depositPercent, project_address: buyer.projectAddress || selectedProjectAddress }).eq('id', dealId)
        if (updateError) throw updateError
      } else {
        const { data, error: dealError } = await supabase.from('deals').insert({ contact_id: savedContactId, sales_rep_id: employee.id, base_amount: unit.price, delivery_amount: deliveryAmount, deposit_percent: depositPercent, project_address: buyer.projectAddress || selectedProjectAddress, project_name: `${selectedDisplayName ?? `${buyer.firstName} ${buyer.lastName}`.trim()} — ${unit.name}` }).select('id,deal_number').single()
