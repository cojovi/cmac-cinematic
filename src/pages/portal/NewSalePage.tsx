...
import { ConfigurationState, PortalError } from '../../components/portal/AsyncState'
...
export default function NewSalePage() {
  ...
  async function prepareContract() {
    if (!unitConfirmed) { setError('Replace the model reference with a confirmed real unit ID before preparing a contract.'); return }
    if (!selectedTemplate?.is_active || !selectedTemplate.provider_template_id) { setError('Select an active legal template with a configured DocuSign template ID.'); return }
    if (!dealId) { setError('Save the deal draft before preparing a contract.'); return }
    if (previewMode || !supabase) { setMessage('DocuSign is not invoked in local preview.'); return }
    setBusy(true)
    const { data, error: functionError } = await supabase.functions.invoke('send-docusign-envelope', { body: { deal_id: dealId, template_id: templateId } })
    setBusy(false)
    if (functionError) setError(functionError.message); else setMessage(String((data as { message?: string })?.message ?? 'Contract preparation requested.'))
  }
  ...
  return <section className="new-sale-page" aria-labelledby="new-sale-heading">
    ...
    <div className="portal-content-grid"><div className="portal-panel">
      ...
      {step === 3 ? <><div className="portal-panel-heading"><div><span>STEP 03 / LEGAL METADATA</span><h2>Transaction template</h2><p>Only admin-approved, provider-mapped templates can create an envelope.</p></div></div><div className="document-list">{templates.rows.map((template) => <label key={String(template.id)} className={templateId === template.id ? 'document-row selected' : 'document-row'}><input type="radio" checked={templateId === template.id} onChange={() => setTemplateId(String(template.id))} /><span className="document-check">{templateId === template.id ? <Check size={14} /> : null}</span><FileSignature size={20} /><span className="document-copy"><strong>{String(template.title)}</strong><small>{String(template.description ?? '')}</small></span><span className={`document-group ${template.is_active ? 'group-required' : ''}`}>{template.is_active ? 'Active' : 'Placeholder'}</span></label>)}</div><div className="real-unit-confirmation"><ShieldCheck size={20} /><div><h3>Replace model reference before legal send</h3><p>Enter a verified operational unit ID supplied by authorized CMAC staff or the future Bolt unit feed.</p><div><label>Real unit / job ID<input value={realUnitId} onChange={(event) => { setRealUnitId(event.target.value); setUnitConfirmed(false) }} /></label><label>Product type<input value={realProductType} onChange={(event) => { setRealProductType(event.target.value); setUnitConfirmed(false) }} /></label><button className="portal-secondary-button" type="button" disabled={!realUnitId.trim() || !realProductType.trim()} onClick={() => setUnitConfirmed(true)}>{unitConfirmed ? <Check size={15} /> : <ShieldCheck size={15} />} {unitConfirmed ? 'Confirmed for draft' : 'Confirm identity'}</button></div></div></div><div className="portal-panel-actions"><button className="portal-text-button" type="button" onClick={back}><ArrowLeft size={15} /> Back</button><button className="portal-primary-button" disabled={!templateId} onClick={() => next()} type="button">Review transaction <ArrowRight size={15} /></button></div></> : null}
      {step === 4 ? <><div className="portal-panel-heading"><div><span>STEP 04 / REVIEW</span><h2>Save the transaction</h2><p>Quote and contract actions remain explicit and never simulate success.</p></div><span className="panel-count">{dealNumber ?? 'Unsaved draft'}</span></div><div className="sale-review-grid"><article><UserRound size={18} /><span>Customer</span><strong>{String(selectedContact?.display_name ?? `${buyer.firstName} ${buyer.lastName}`.trim())}</strong><small>{String(selectedContact?.email ?? buyer.email)}</small></article><article><Home size={18} /><span>Unit reference</span><strong>{unitConfirmed ? realUnitId : unit?.id}</strong><small>{unitConfirmed ? 'Manual / confirmed' : 'Mock / draft only'}</small></article><article><ReceiptText size={18} /><span>Transaction value</span><strong>{money.format(total)}</strong><small>{depositPercent}% deposit · {money.format(total * depositPercent / 100)}</small></article><article><FileSignature size={18} /><span>Template</span><strong>{String(selectedTemplate?.title ?? 'Not selected')}</strong><small>{selectedTemplate?.is_active ? 'Provider ready' : 'Placeholder / inactive'}</small></article></div><div className="portal-panel-actions review-actions"><button className="portal-text-button" type="button" onClick={back}><ArrowLeft size={15} /> Back</button><div><button className="portal-secondary-button" disabled={busy} type="button" onClick={() => void createQuote()}><ReceiptText size={15} /> Create quote</button><button className="portal-primary-button" disabled={busy} type="button" onClick={() => void prepareContract()}><FileSignature size={15} /> Prepare contract</button></div></div></> : null}
