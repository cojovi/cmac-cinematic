...
import { ArrowLeft, CalendarPlus, CheckCircle2, MessageSquarePlus, Save, ShieldCheck } from 'lucide-react'
...
import RecordFields from '../../components/portal/RecordFields'
import ActivityFeed from '../../components/portal/ActivityFeed'
import ContactAssignmentPanel from '../../components/portal/ContactAssignmentPanel'
...
export default function ResourceDetailPage({ resource }: { resource: DetailResource }) {
  ...
  const recordQuery = usePortalRows(tableByResource[resource], {
    ...
    select: resource === 'leads'
      ? '*,contacts(first_name,last_name,display_name,email,phone,project_address),assigned_employee:employees!leads_assigned_employee_id_fkey(display_name,rep_code),owner:employees!leads_assigned_employee_id_fkey(display_name,rep_code)'
      : resource === 'customers' ? '*,owner:employees!contacts_assigned_employee_id_fkey(display_name,rep_code),creator:employees!contacts_created_by_fkey(display_name,rep_code)'
      : '*,contacts(display_name,email,phone),owner:employees!deals_sales_rep_id_fkey(display_name,rep_code)',
  })
  const activityFilter = resource === 'customers' ? 'contact_id' : resource === 'leads' ? 'lead_id' : 'deal_id'
  const activityQuery = usePortalRows('activities', { select: activitySelect, filter: { column: activityFilter, value: id }, orderBy: 'created_at', limit: 50 })
  const dealUnitsQuery = usePortalRows('deal_units', { enabled: resource === 'deals', filter: { column: 'deal_id', value: id }, orderBy: 'created_at', limit: 50 })
  const contractsQuery = usePortalRows('contracts', { enabled: resource === 'deals', filter: { column: 'deal_id', value: id }, orderBy: 'created_at', limit: 50 })
  const linkedDealsQuery = usePortalRows('deals', { enabled: resource !== 'deals', filter: { column: resource === 'customers' ? 'contact_id' : 'lead_id', value: id }, orderBy: 'created_at', limit: 50 })
  ...
  const reloadAll = async () => { await Promise.all([recordQuery.reload(), activityQuery.reload(), dealUnitsQuery.reload(), contractsQuery.reload(), linkedDealsQuery.reload()]) }
  async function performAction(action: () => Promise<void>) {
    if (pendingAction) return
    setPendingAction(true); setActionMessage(null)
    try { await action() } catch { setActionMessage('The update could not be completed. Please try again.') }
  ...
  if (recordQuery.error || activityQuery.error || dealUnitsQuery.error || contractsQuery.error || linkedDealsQuery.error) return <PortalError message={recordQuery.error ?? activityQuery.error ?? dealUnitsQuery.error ?? contractsQuery.error ?? linkedDealsQuery.error ?? 'Unable to open record.'} retry={() => void reloadAll()} />
  ...
  return (
    <section className="record-detail-page">
      ...
      <div className="record-detail-grid">
        ...
        <RecordFields record={record} resource={resource} deals={linkedDealsQuery.rows} />
        <aside className="record-actions-stack">
          {resource === 'customers' && employee?.role === 'admin' ? <ContactAssignmentPanel key={`${id}-${String(record.assigned_employee_id)}`} record={record} onSaved={reloadAll} /> : null}
          <form className="workspace-card record-action-form" onSubmit={event => { event.preventDefault(); void performAction(() => addNote(event)) }}><div className="workspace-card-heading"><div><span>TIMELINE / NOTE</span><h3>Add context</h3></div><MessageSquarePlus size={19} /></div><label><span>Note</span><textarea value={note} onChange={(event) => setNote(event.target.value)} rows={4} placeholder="Customer conversation, site context, or next step" required /></label><button className="portal-primary-button" disabled={previewMode || pendingAction} type="submit"><Save size={15} /> Save note</button></form>
          <form className="workspace-card record-action-form" onSubmit={event => { event.preventDefault(); void performAction(() => addTask(event)) }}><div className="workspace-card-heading"><div><span>FOLLOW-UP / TASK</span><h3>Schedule action</h3></div><CalendarPlus size={19} /></div><label><span>Task</span><input value={taskTitle} onChange={(event) => setTaskTitle(event.target.value)} required /></label><label><span>Due</span><input type="datetime-local" value={dueAt} onChange={(event) => setDueAt(event.target.value)} required /></label><button className="portal-secondary-button" disabled={previewMode || pendingAction} type="submit"><CalendarPlus size={15} /> Add follow-up</button></form>
          {resource === 'deals' ? <article className="workspace-card record-action-form sale-completion-card"><div className="workspace-card-heading"><div><span>ATTRIBUTION / FINAL</span><h3>Mark deal sold</h3></div><CheckCircle2 size={19} /></div><dl><div><dt>Confirmed units</dt><dd>{dealUnitsQuery.rows.filter((unit) => unit.source !== 'mock' && unit.confirmed_at).length} / {dealUnitsQuery.rows.length}</dd></div><div><dt>Completed contracts</dt><dd>{contractsQuery.rows.filter((contract) => contract.status === 'completed').length}</dd></div></dl>{employee?.role === 'admin' && contractsQuery.rows.every((contract) => contract.status !== 'completed') ? <label><span>Admin override reason · minimum 10 characters</span><textarea value={overrideReason} onChange={(event) => setOverrideReason(event.target.value)} rows={3} /></label> : null}<button className="portal-primary-button" disabled={previewMode || pendingAction || record.status === 'won' || dealUnitsQuery.rows.length === 0 || dealUnitsQuery.rows.some((unit) => unit.source === 'mock' || !unit.confirmed_at) || (contractsQuery.rows.every((contract) => contract.status !== 'completed') && !(employee?.role === 'admin' && overrideReason.trim().length >= 10))} type="button" onClick={() => void performAction(markSold)}><CheckCircle2 size={15} /> Create unit attribution</button></article> : null}
        </aside>
        <article className="workspace-card record-timeline"><div className="workspace-card-heading"><div><span>ACTIVITY / AUDITABLE</span><h3>Timeline</h3></div><MessageSquarePlus size={19} /></div><ActivityFeed rows={activityQuery.rows} /></article>
      </div>
    </section>
  )
}
