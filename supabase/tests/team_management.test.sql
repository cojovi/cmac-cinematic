...
select is((select l.assigned_employee_id::text from public.leads l join public.contacts c on c.id=l.contact_id where c.email='team.qa.new@example.com'),'91000000-0000-4000-8000-000000000001','new lead retains the administrator owner');
select lives_ok($$select public.manage_lead('91000000-0000-4000-8000-000000000001','update',(select l.id from public.leads l join public.contacts c on c.id=l.contact_id where c.email='team.qa.new@example.com'),'{"assigned_employee_id":"91000000-0000-4000-8000-000000000002"}')$$,'admin-owned lead can be reassigned to a salesperson');
select lives_ok($$select public.manage_lead('91000000-0000-4000-8000-000000000001','update',(select l.id from public.leads l join public.contacts c on c.id=l.contact_id where c.email='team.qa.new@example.com'),'{"assigned_employee_id":"91000000-0000-4000-8000-000000000001"}')$$,'salesperson-owned lead can be reassigned to an administrator');
select is((select assigned_employee_id::text from public.contacts where email='team.qa.new@example.com'),'91000000-0000-4000-8000-000000000001','contact ownership follows administrator reassignment');
select is((select l.assigned_employee_id::text from public.leads l join public.contacts c on c.id=l.contact_id where c.email='team.qa.new@example.com'),'91000000-0000-4000-8000-000000000001','lead ownership saves administrator reassignment');
select throws_ok($$select public.manage_lead('91000000-0000-4000-8000-000000000002','update',(select l.id from public.leads l join public.contacts c on c.id=l.contact_id where c.email='team.qa.new@example.com'),'{"assigned_employee_id":"91000000-0000-4000-8000-000000000002"}')$$,'42501','Administrator access is required to edit or convert leads','salespeople still cannot reassign a lead');
select lives_ok($$select public.manage_lead('91000000-0000-4000-8000-000000000001','convert',(select l.id from public.leads l join public.contacts c on c.id=l.contact_id where c.email='team.qa.new@example.com'),'{"assigned_employee_id":"91000000-0000-4000-8000-000000000001"}')$$,'administrator-owned lead can convert into a deal');
select is((select d.sales_rep_id::text from public.deals d join public.contacts c on c.id=d.contact_id where c.email='team.qa.new@example.com'),'91000000-0000-4000-8000-000000000001','converted deal retains administrator ownership');
