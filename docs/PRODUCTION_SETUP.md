...
7. Turn on the project-level **Allow new users to sign up** setting. This must be enabled for a first-time Google identity to reach the database hook; the hook remains the authoritative domain gate and rejects every non-Google, non-Container, inactive, or unmapped identity.
8. The Before User Created hook is configured at `pg-functions://postgres/private/before_user_created_hook`. It admits Google identities in the exact `cmaccontainers.com` domain plus server-managed exceptional mappings in `private.employee_google_identities`. A first-time Container-domain user is automatically created as an active `sales_rep`; Google profile metadata cannot set roles or active status. An exceptional identity only links its preselected employee and cannot create a role. Existing rows preserve their administrator-managed role and status, and an explicitly deactivated employee remains blocked.
9. Request identity only: OpenID, email, and profile.
10. Confirm a new Container-domain account is provisioned as `sales_rep`, Cody's mapped `codyv@cmacroofing.com` identity links only to his administrator record, another Roofing-domain account is rejected, a deactivated employee is rejected, and every non-Google account is rejected.
