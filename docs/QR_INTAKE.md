...
- The earlier setup-test inquiry `60dd5a49-52f4-45d8-bdb7-7f35d1394ae6` initially remained `not_configured`. After real Google administrator login, the production QR tab's confirmed retry succeeded: its attempt count is two (one pre-configuration failure, one confirmed send). All three clearly labeled QA inquiries now show sent, with no remaining notification attention. Test visits/inquiries remain included in the initial reporting totals; no real prospective-customer data was used, no test rows were deleted, and no status was fabricated.
- Latest automated checks: lint, build, both Edge Function type checks, **57 unit/API tests**, **140 pgTAP assertions**, duplicate-request concurrency tests, and **76 browser tests** at 375/768/1024/1440 pixels. Automated browser authentication and successful Gmail responses use controlled fixtures, not real employee credentials. Separate live browser checks verified production Google admin sign-in, reporting, employee summaries, and Gmail sends/retry.
- Production deployment of `61027a5` reached READY, `/qr` and the admin route served successfully, and unauthenticated `/api/inventory` returned 401. A full browser submission on the public production form also passed: inquiry `3b6f9987-4776-478d-83f1-21538d3b8b3d` received Gmail confirmation at 20:55 UTC. Final administrator testing identified the old Vercel-only Supabase redirect configuration; the owner approved correcting the www/apex callback addresses and canonical site URL. See [production setup](PRODUCTION_SETUP.md#september-23-2026-release-update).
- After the live redirect configuration correction, Cody's Google account completed a single sign-in directly into the production employee portal; the live QR metrics and all answers were readable by the administrator, and the Employees page showed current company-wide ownership and activity. No real employee role or customer assignment was changed during verification.
